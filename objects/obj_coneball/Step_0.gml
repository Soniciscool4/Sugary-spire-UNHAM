...
with (obj_player)
{
    if (state == (58 << 0) || state == (53 << 0) || state == (16 << 0) || state == (74 << 0) || state == (105 << 0) || (state == (85 << 0) && sprite_index == spr_cottontransfo) || place_meeting(x, y, obj_secretportal))
        _move = false
}
if (image_alpha >= 1)
{
...
if (playerid.state == (119 << 0))
    maxspeed = 0
if (_move && place_meeting(x, y, playerid) && playerid.state != (105 << 0) && (!instance_exists(obj_fadeout)) && (!instance_exists(obj_endlevelfade)) && image_alpha >= 1)
{
    with (obj_player)
    {
