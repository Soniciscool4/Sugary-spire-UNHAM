...
if instance_exists(obj_player)
{
    with (obj_player)
    {
        if (other.fadein && (obj_player.state == (74 << 0) || obj_player.state == (58 << 0)) && (place_meeting(x, y, par_door) || place_meeting(x, y, obj_startgate)))
        {
            state = (59 << 0)
            image_index = 0
        }
        if (other.fadein && obj_player.state == (74 << 0) && (obj_player.sprite_index == spr_player_downpizzabox || obj_player.sprite_index == spr_player_uppizzabox || obj_player.sprite_index == spr_pizzano_downpizzabox || obj_player.sprite_index == spr_pizzano_uppizzabox))
            state = (63 << 0)
        if (baddiegrabbedID != noone && state != (74 << 0) && state != (58 << 0))
        {
