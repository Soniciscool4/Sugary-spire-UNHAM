if (global.panic && room != hub_start)
{
    with (obj_player)
    {
        if (grounded && key_up && (state == (0 << 0) || state == (65 << 0) || state == (66 << 0) || state == (67 << 0)))
        {
            targetDoor = "none"
            ...
            ini_close()
            function confecti_count_level(argument0) //confecti_count_level@gml_Object_obj_exitgate_Collision_obj_player
            {
                var cnt = 0
                ini_open(global.charsave)
                for (var i = 1; i <= 5; i++)
