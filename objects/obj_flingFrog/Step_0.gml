...
if (place_meeting(x, y, _player) && (!(scr_solid(x, y))) && _player.state != (96 << 0) && grabbedPlayer <= noone && waitTimer <= 0)
{
    ...
    with (grabbedPlayer)
    {
        if (state == (86 << 0) || (state == (57 << 0) && (sprite_index == spr_playerPZ_scooterspinstart || sprite_index == spr_playerPZ_scooterspin || sprite_index == spr_pizzano_wallclimbspin)))
        {
            movespeed = abs(hsp)
            if (move != 0)
                xscale = move
